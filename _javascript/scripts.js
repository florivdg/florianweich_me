...
$(document).ready(function() {
  ...
  $('a[href^="#"]').on('click', function(e) {
    ...
    if ($mainNav.hasClass('shown')) {
      $mainNav
        .removeClass('shown')
        .animateCss('fadeOutLeftBig', 'hidden-mobile');
    }
  ...
  */
  ...
  $('#contactform').on('submit', function(e) {
    ...
    if (
      ...
      $.ajax({
        url: 'https://mailservice.florianweich.me/api/sendmail',
        type: 'POST',
        headers: {
          Authorization: 'Bearer 38b446a0d7a343dba61c4135a6c537e6',
        ...
        data: JSON.stringify({
          ...
          content: $('#contactform textarea[name=message]').val(),
          name: $('#contactform input[name=name]').val(),
          privacypolicy:
            $('#contactform input[name=privacypolicy]').val() == 'on'
              ? true
              : false
        })
      })
...
var $animation_elements = $(
  '.collection-item, .references-list-item, .btn-animated'
);
var $window = $(window);
